...
final readonly class SubscriberSkippedEvent implements EngineEvent
{
    ...
    {
        return sprintf('Subscriber "%s" for "%s" skipped event "%s" (position: %d) because it is already father ahead (position: %d)', $this->subscriber->handler::class, $this->subscription->id->value, get_debug_type($this->event), $this->event->position()->value, $this->subscription->position->value);
    }
}